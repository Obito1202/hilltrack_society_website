...
const NAV = [
  ...
  { to: "/members", label: "Governing Body" }, { to: "/events", label: "Events" },
  { to: "/achievements", label: "Achievements" }, { to: "/schools", label: "Schools" },
      ...
      <header className="border-b" style={{ borderColor: "var(--line)" }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-5 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {NAV.map(n => (
              <NavLink key={n.to} to={n.to} end={n.to === "/"} className={({isActive}) => `nav-link text-sm ${isActive ? "active" : ""}`} style={{ color: "var(--ink)" }}>{n.label}</NavLink>
            ))}
          </nav>
        ...
        <div className="lg:hidden border-t overflow-x-auto" style={{ borderColor: "var(--line)" }}>
          <div className="px-6 py-3 flex gap-5 whitespace-nowrap">
            {NAV.map(n => <NavLink key={n.to} to={n.to} end={n.to === "/"} className="text-xs uppercase tracking-widest" style={{ color: "var(--ink-soft)" }}>{n.label}</NavLink>)}
          </div>
        </div>
      </header>
      ...
      <footer className="mt-24 border-t" style={{ borderColor: "var(--line)" }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-14 grid md:grid-cols-3 gap-10">
          <div>
            ...
            <p className="mt-4 text-sm" style={{ color: "var(--ink-soft)" }}>Empowering Himalayan communities through education, sustainable energy, and environmental stewardship.</p>
          </div>
          <div>
            <div className="overline">Registered Office</div>
            <p className="mt-3 text-sm" style={{ color: "var(--ink-soft)" }}>Joshi Niwas, Malla Krishnapur,<br/>Tallital, Nainital, Uttarakhand</p>
          </div>
          <div>
        ...
        <div className="border-t" style={{ borderColor: "var(--line)" }}>
          <div className="max-w-7xl mx-auto px-6 lg:px-12 py-5 text-xs" style={{ color: "var(--ink-soft)" }}>© {new Date().getFullYear()} Hilltrack Society. All rights reserved.</div>
        </div>
      </footer>
    </div>
  );
